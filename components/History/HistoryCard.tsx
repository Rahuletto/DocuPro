...
export default function HistoryCard({ text }: { text: string }) {
  ...
  return (
    <p
      ...
      className={`${text.includes("æ") ? "border-dotted" : "border-dashed"} cursor-pointer flex items-center line-clamp-1 justify-between gap-2 transition duration-200 dark:active:bg-dark-accent-background active:bg-light-accent-background px-4 md:text-lg text-md dark:opacity-40 opacity-60 py-2 rounded-xl font-medium border-2 hover:border-light-accent-color dark:hover:border-dark-accent-color hover:text-light-accent-color dark:hover:text-dark-accent-color hover:opacity-80 text-light-color dark:text-dark-color`}
    >
      {text.replace("æ", "")}
